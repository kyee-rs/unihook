...
P.toFraction = function (maxD) {
    ...
    n0.s = n1.s = x.s;
    r = divide(n1, d1, e, 1).minus(x).abs().cmp(
            divide(n0, d0, e, 1).minus(x).abs(),
        ) < 1
        ? [n1, d1]
        : [n0, d0];
    Ctor.precision = pr;
...
function checkRoundingDigits(d, i, rm, repeating) {
    ...
    if (repeating == null) {
        if (i < 3) {
            ...
            r = rm < 4 && rd == 99999 || rm > 3 && rd == 49999 || rd == 5e4 ||
                rd == 0;
        } else {
...
function finalise(x, sd, rm, isTruncated) {
    ...
    if (sd != null) {
        ...
        isTruncated = isTruncated || sd < 0 || xd[xdi + 1] !== void 0 ||
            (j < 0 ? w : w % mathpow(10, digits - j - 1));
        roundUp = rm < 4
            ? (rd || isTruncated) && (rm == 0 || rm == (x.s < 0 ? 3 : 2))
            : rd > 5 ||
                rd == 5 &&
                    (rm == 4 || isTruncated || rm == 6 && (i > 0
                                            ? j > 0
...
function naturalExponential(x, sd) {
    ...
    for (;;) {
        ...
        if (
            ...
            if (sd == null) {
                if (
                    rep < 3 && checkRoundingDigits(sum2.d, wpr - guard, rm, rep)
                ) {
